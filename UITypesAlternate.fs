...
let createFolderELM (n :Node) (dinfo :DirectoryInfo) (engine :Engine<UIMsg,UIState>)=
    ...
    let chg = v.Radius |> Observable.add (fun x -> engine.Post(Resize(n.key,x)))
    let scale = v.Scale
    let pos = v.Position
...
let  CreateTreeView (tree :Tree) =
    ...
    let active msg (state :UIState) =
        ...
        | Resize(key,size) -> let index = state.nodes |> Array.findIndex (fun x -> x.key = key)
                              let zed = {state.nodes.[index] with data={state.nodes.[index].data with size=size}}
                              state.nodes.[index] <- zed
...
let createWindow () =
    ...
    let nil_data = {position=0.0,0.0;size=1.0;distance=0.0;ar=0.0<rad>;angle=0.0<rad>;parent= -1;}
    let nil_node = {key=0; data=nil_data; ntype=TRoot; relscale=1.0; element=Folder (DirectoryInfo "C:"); extsize=0.0} 
    let orgin = [| {key=0; data={nil_data with parent= -1;}; ntype=TRoot; relscale=1.0; element=Folder (DirectoryInfo "C:"); extsize=0.0} 
                   {key=1; data={nil_data with parent= 0;}; ntype=TLeaf; relscale=1.0; element=Folder (DirectoryInfo "C:\Program Files"); extsize=0.0 }
                |] 
