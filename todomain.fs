...
type VM() =
    ...
    member this.List with get () = 
                        ...
                        | Some(ary) -> ary |>  Array.map (fun x -> 
                                ...
                                prev = property ((x.prev :> obj) :?> string)}) //toJson -> ofJson doesnt work for Property nor DateTime 
...
let item_view (vm :VM) (index :int) (r :Reminder) =
    ...
    let charlimit = span (attr [cls ("inner-status " + (if r.editing.get then "show" else "hide"))]) [charlim.ToString() + " characters left"]
    let description = div (attr [
                            cls ("description " + (if r.editing.get then "hide" else "")) ;
    ...
    let edit = textarea (attr [
                            cls (if r.editing.get then "show" else "hide") ;
                            css [("height","29px"); ("min-height","0px")]
                            prop "rows" 1;
    ...
    let lbl = label (attr [prop "data-date" r.date])
                [
                    ...
                    edit;
                    charlimit;
                    span (attr [cls "date"]) [ ago]

                ]
    li None [
        lbl ;
        span (attr [cls "destroy";
                    onClick (fun e -> 
                        vm.List <- vm.List 
    ...
    section (attr [prop "id" "reminderapp"]) [
        div (attr [cls "tasks-Container"]) [
            ...
            div (attr [prop "id" "input-status"]) 
                ...
                else [ span (attr [cls "danger"]) ["limit of " + vm1.MaxString.ToString()]]) ;
        ]
